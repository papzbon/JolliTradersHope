...
namespace JolliTradersHope.Api.Data.Entities
{
    ...
    public class Product
    {
        ...
        public Product(int id, string code, string name, string? image, decimal price, string unit, short categoryId)
        {
            Id = id;
            Code = code;
            Name = name;
        ...
        internal static readonly Expression<Func<Product, ProductDto>> DtoSelector =
            p => new ProductDto(p.Id, p.Code, p.Name, p.Image, p.Price, p.Unit, p.CategoryId);
    }
}
